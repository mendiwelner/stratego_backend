...
class GameManage:
    ...
    async def player_connect_game(player_websocket: WebSocket) -> None:
        ...
        player_name = GameManage.get_name_from_user_id(user_id)
        player_id = await game.connect_player(player_websocket, player_name)
        await GameManage.process_player_to_game_connection(player_websocket, player_id, game)

    @staticmethod
    def get_name_from_user_id(user_id: int,  db: Session = DBSessionManager.get_db()) -> str:
